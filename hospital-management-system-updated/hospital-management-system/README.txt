...
- Appointments have their own screen: pick a patient (search by name, ID or phone),
  pick a date on a month calendar (days with appointments are marked), and see that
  day's schedule; "Show all dates" lists everything. Past dates and double-bookings
  of the same patient/doctor/day are rejected.
...
GUI:
  ...
  AppointmentPanel.java            - book + view appointments (patient search, calendar, day schedule)
  CalendarPicker.java              - month-view date picker (marks days that have appointments)
  LinkButton.java                  - flat text button (calendar arrows, "Clear")
