...
public class PatientDAO {
    ...
    /** Convenience overload for callers that only have a YYYY-MM-DD string. */
    public void bookAppointment(int patientId, String doctor, String date) throws HospitalException {
        try {
            bookAppointment(patientId, doctor, LocalDate.parse(date));
        } catch (DateTimeParseException e) {
            throw new HospitalException("Invalid date! Use YYYY-MM-DD.");
        }
    }

    public void bookAppointment(int patientId, String doctor, LocalDate date) throws HospitalException {
        String checkSql = "SELECT id FROM patients WHERE id = ?";
        String duplicateSql = "SELECT 1 FROM appointments WHERE patient_id = ? AND doctor = ? AND appointment_date = ?";
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            try (PreparedStatement dup = con.prepareStatement(duplicateSql)) {
                dup.setInt(1, patientId);
                dup.setString(2, doctor);
                dup.setDate(3, Date.valueOf(date));
                try (ResultSet rs = dup.executeQuery()) {
                    if (rs.next()) {
                        throw new HospitalException("This patient already has an appointment with "
                                + doctor + " on " + date + ".");
                    }
                }
            }
    ...
    /** Distinct doctor names already used in appointments (for suggestions in the booking form). */
    public List<String> getDoctors() throws HospitalException {
        List<String> doctors = new ArrayList<>();
        String sql = "SELECT DISTINCT doctor FROM appointments ORDER BY doctor";
        try (Connection con = DBConnection.getConnection();
             Statement st = con.createStatement();
             ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) doctors.add(rs.getString("doctor"));
        } catch (SQLException e) {
            throw new HospitalException("Unable to fetch doctors: " + e.getMessage());
        }
        return doctors;
    }
    ...
    public List<Appointment> getAllAppointments() throws HospitalException {
        ...
        String sql = "SELECT a.appointment_id, a.patient_id, p.name, a.doctor, a.appointment_date " +
                     "FROM appointments a JOIN patients p ON a.patient_id = p.id ORDER BY a.appointment_date, a.appointment_id";
        try (Connection con = DBConnection.getConnection();
             Statement st = con.createStatement();
             ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) {
